...
押し目買い（現値より下に `buy limit` を多数配備）の運用で、**ドル円が急落（例: 円買い介入で 3 円下落）したときに証拠金維持率が安全圏に残るか**を試算します。下落の過程で**発動する `buy limit` を約定建玉として織り込む**ため、建玉・ロット・含み損・必要証拠金の増加を反映できます。

```powershell
# USDJPY が 3 円下落した場合を主判定し、0〜15円のシナリオ表も表示
python check_margin_stress.py --symbol USDJPY --drop 3 --max-drop 15 --step 1
```

出力内容:

- 現在の口座状況（残高・有効証拠金・使用/余剰証拠金・証拠金維持率、ロスカット/マージンコール水準）
- 指定下落幅での**判定**（✅安全圏 / ⚠️マージンコール / 🛑ロスカット）と、発動する待機注文の件数・追加ロット
- **下落シナリオ表**（下落幅ごとの維持率と状態）
- **余裕度**（現値からあと何円下げるとマージンコール/ロスカットに抵触するか）

金額は MT5 の `order_calc_profit` / `order_calc_margin` で通貨換算・レバレッジを反映して計算します。ロスカット/マージンコール水準は口座情報（サーバー値）を使用し、`--stop-out-level` / `--margin-call-level` で上書きできます。

> ⚠️ スプレッド・スワップ・約定スリッページ等は簡略化しています。実際の余力確認の**目安**としてご利用ください。
...
├── check_margin_stress.py  # 価格下落ストレステスト(証拠金維持率)
├── mt5_oanda/
│   ├── __init__.py
...
│   ├── stats.py            # t検定・符号検定・信頼区間(外部依存なし)
│   └── margin_stress.py    # 下落ストレステスト(待機注文発動を考慮)
├── requirements.txt
├── .env.example
├── .gitignore
├── LICENSE
